...
from easy_agent.core.state import (
    AgentState,
...
from easy_agent.utils.message_utils import validate_chat_history
from easy_agent.utils.model_utils import should_bind_tools
from easy_agent.utils.prompt_utils import (
    Prompt,
    StateSchemaType,
...
def create_react_agent(
    ...
    *,
    ...
    runtime_injection_params: dict = {},
    runtime_injection_func: Any = None,
    ...
    if should_bind_tools(model, tool_classes) and tool_calling_enabled:
        model = cast(BaseChatModel, model).bind_tools(tool_classes)

    if runtime_injection_func and runtime_injection_params:
        model = model | RunnableLambda(
            lambda x: runtime_injection_func(x, **runtime_injection_params)
        )

    model_runnable = get_prompt_runnable(prompt) | model
    ...
    def _are_more_steps_needed(state: StateSchema, response: BaseMessage) -> bool:
        ...
        remaining_steps = get_state_value(state, "remaining_steps", None)
        is_last_step = get_state_value(state, "is_last_step", False)
        return (
    ...
    def _get_model_input_state(state: StateSchema) -> StateSchema:
        if pre_model_hook is not None:
            messages = (
                get_state_value(state, "llm_input_messages")
            ) or get_state_value(state, "messages")
        ...
        validate_chat_history(messages)
    ...
    def route_tool_responses(state: StateSchema) -> str:
        for m in reversed(get_state_value(state, "messages")):
